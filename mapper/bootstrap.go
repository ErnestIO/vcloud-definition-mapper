...
func GenerateBootstraps(instances []output.Instance) []output.Execution {
	var bootstraps []output.Execution

	// Add instance to bootstrap if not salt-master
	for _, instance := range instances {
		if instance.IP.String() != "10.254.254.100" {
			bootstraps = append(bootstraps, output.Execution{
...
func GenerateBootstrapCleanup(instances []output.Instance) []output.Execution {
	var bootstraps []output.Execution
