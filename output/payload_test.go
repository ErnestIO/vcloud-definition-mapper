		...
		Convey("When diffing a fsm message", func() {
			Convey("And there is no previous result", func() {
				...
				Convey("Then it should create executions and bootstraps", func() {
					...
					So(m.InstancesToCreate.Items[1].NetworkName, ShouldEqual, "network")
					So(m.InstancesToCreate.Items[1].IP.String(), ShouldEqual, "10.64.0.102")
					So(len(m.Bootstraps.Items), ShouldEqual, 2)
					So(m.Bootstraps.Items[0].Name, ShouldEqual, "Bootstrap test-1")
					So(m.Bootstraps.Items[0].Payload, ShouldEqual, "/usr/bin/bootstrap -master 10.254.254.100 -host 10.64.0.101 -username ernest -password 'b00tStr4pp3rR' -max-retries 20 -minion-name test-1")
					So(m.Bootstraps.Items[0].Target, ShouldEqual, "list:salt-master.localdomain")
					So(m.Bootstraps.Items[1].Name, ShouldEqual, "Bootstrap test-2")
					So(m.Bootstraps.Items[1].Payload, ShouldEqual, "/usr/bin/bootstrap -master 10.254.254.100 -host 10.64.0.102 -username ernest -password 'b00tStr4pp3rR' -max-retries 20 -minion-name test-2")
					So(m.Bootstraps.Items[1].Target, ShouldEqual, "list:salt-master.localdomain")
			...
			Convey("And instance count has increased but the execution payload is the same", func() {
				...
				Convey("Then it should create executions and bootstraps for only the new instance", func() {
					So(len(m.InstancesToCreate.Items), ShouldEqual, 1)
					So(len(m.Bootstraps.Items), ShouldEqual, 1)
					So(m.Bootstraps.Items[0].Name, ShouldEqual, "Bootstrap test-2")
					So(m.Bootstraps.Items[0].Payload, ShouldEqual, "/usr/bin/bootstrap -master 10.254.254.100 -host 10.64.0.102 -username ernest -password 'b00tStr4pp3rR' -max-retries 20 -minion-name test-2")
					So(m.Bootstraps.Items[0].Target, ShouldEqual, "list:salt-master.localdomain")
					So(len(m.ExecutionsToCreate.Items), ShouldEqual, 1)
					So(m.ExecutionsToCreate.Items[0].Name, ShouldEqual, "Execution")
					So(m.ExecutionsToCreate.Items[0].Payload, ShouldEqual, "date")
